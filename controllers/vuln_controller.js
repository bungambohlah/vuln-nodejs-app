...
const sqli_check_train_get = (req, res) => {
  ...
  const q =
    "SELECT ntrains FROM trains where from_stnt='" +
    from +
    "' and to_stnt='" +
    to +
    "';";
...
const fixed_sqli_check_train_get = (req, res) => {
  const from = req.params.from;
  const to = req.params.to;
  Train.findAll({ where: { from_stnt: from, to_stnt: to } })
    .then((trains) => res.send(trains))
...
const xxe_comment = (req, res) => {
  ...
  const parsecomment = libxmljs.parseXmlString(rawcomment, {
    noent: true,
    noblanks: true,
  });
...
const userNotes_get = (req, res) => {
  ...
  Notes.findAll({ where: { userid: userid } }).then((queryResult) => {
    res.header("Content-Type", "application/json");
...
const ticket_booking_get = (req, res) => {
  ...
  const file = {
    url: `http://localhost:${process.env.HOST_PORT}/ticket/generate_ticket?passenger_name=${req.query.passenger_name}&from_stnt=${req.query.from_stnt}&to_stnt=${req.query.to_stnt}&date=${req.query.date}`,
  };
...
const user_edit_get = (req, res) => {
  Users.findOne({ where: { username: req.user.username } }).then((user) => {
...
const add_user_get = (req, res) => {
  Org.findOne(
    ...
    Users.findAll({
      where: { username: { [Op.ne]: req.user.username } },
      attributes: ["username"],
    })
      .then((users) => {
        res.render("add-user", {
...
const totp_setup_post = (req, res) => {
  ...
  if (!(username in tmp_totpSecret)) {
    ...
    const verification_Token = twofactor.generateToken(
      tmp_totpSecret[username]
    );
    const verify = twofactor.verifyToken(
      tmp_totpSecret[username],
      req.body.totp_verify
    );
    if (verify != null) {
      if (verify.delta == "0") {
        Users.update(
          { totpSecret: tmp_totpSecret[username] },
...
const login_totp_verification_post = (req, res) => {
  if (
    req.user.totpSecret != "" ||
    req.body.totp_code != "" ||
    req.body.totp_code != null ||
    req.body.totp_code == undefined
  ) {
    const verify = twofactor.verifyToken(
      req.user.totpSecret,
      req.body.totp_code
...
const totp_disable_post = (req, res) => {
  ...
  if (req.user.totpSecret === "")
    return res.status(403).send("TOTP is not enabled for this user");
...
const react_xss_get = (req, res) => {
  res.sendFile(
    path.resolve(__dirname, "../vuln_react_app/build", "index.html")
  );
};
...
MongoClient.connect(dbURL, mongodb_config, (err, db) => {
  if (err)
    return console.log(
      "MongoDB connection error: NoSQL exercise will not work " + err
    );
  ...
  dbo.collection("mongodb-notes").insertOne(
    {
      username: "SuperSecretUser",
      noteTitle: "SuperSecretNote",
      noteBody: "YOU HAVE SOLVED THE CHALLENGE",
    },
    (err, result) => {
      if (err)
        return console.log(
          "Internal Error: unable to insert data into mongodb-notes collection"
        );
    }
  );
  ...
  dbo.collection("secret").insertOne(
    {
      password: "SuperSecretPassword!!",
      flag: "$flag{You_have_solved_this_exercise}",
    },
    (err, result) => {
      if (err)
        return console.log(
  ...
  MongoClient.connect(dbURL, mongodb_config, (err, client) => {
    if (err)
      return res
        .status("500")
        .send(
          "MongoDB is not installed, Please follow the installation guideline."
        );
    const db = client.db("vuln_nodejs_app");
    db.collection("mongodb-notes")
...
async function graphql_UpdateProfile(args, req) {
  const updateQuery = `UPDATE users SET email='${args.email}', password='${md5(
    args.password
  )}' WHERE username = '${req.user.username}';`;
  await con.connect();
    ...
    request(
      ...
      (err, response, body) => {
        ...
        const bitcoin_usd_value =
          bitcoin_quantity * body.bitcoin.usd + Math.floor(Math.random() * 100);
...
const secret_post = (req, res) => {
  MongoClient.connect(dbURL, mongodb_config, (err, client) => {
    ...
    db.collection("secret")
      .find({ $where: "this.password =='" + req.body.password + "'" })
      .toArray()
      .then((secret) => {
        if (secret.length == 0)
